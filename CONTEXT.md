...
- **Round finalization contract**: The repo-local `orchestrator/round-finalization-schema.md` file that defines the machine contract for reviewer-authored `review-record.json`, including rejected-review retry targets, and compact controller-authored `closeout-record.json`.
- **Review feedback loop**: The same-round retry path after a rejected review. The reviewer records a machine retry target in `review-record.json`; ordinary fix feedback returns to `implement`, and plan or scope feedback returns to `plan`.
